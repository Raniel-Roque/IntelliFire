                ...
                <div class="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-10 items-start">
                    <section class="lg:col-span-3 order-1" aria-label="Latest notifications">
                        ...
                        <div id="fire-exit-maps-mobile" class="mt-8 lg:hidden scroll-mt-24">
                            ...
                            <div class="mt-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4" x-data="{ selectedRoom: 'Room 101', isUrgent: false, latestLevel: 'warning', selectedLevel: null, mapsBaseUrl: '{{ url('/maps') }}', mapUrl(room) { const r = encodeURIComponent(String(room || '').trim()); if (!r) return ''; return `${this.mapsBaseUrl}/${r}${this.isUrgent ? '.png' : '_W.jpg'}`; }, get selectedUrl() { return this.mapUrl(this.selectedRoom); } }" @select-fire-exit-map.window="if ($event.detail && $event.detail.roomName) selectedRoom = $event.detail.roomName" @latest-emergency-level.window="latestLevel = ($event.detail && String($event.detail.level || '').toLowerCase()) || 'warning'; if (selectedLevel === null) isUrgent = (latestLevel === 'urgent')" @selected-emergency-level.window="selectedLevel = ($event.detail && String($event.detail.level || '').toLowerCase()) || 'warning'; isUrgent = (selectedLevel === 'urgent')" @clear-selected-emergency-level.window="selectedLevel = null; isUrgent = (String(latestLevel || 'warning').toLowerCase() === 'urgent')">
